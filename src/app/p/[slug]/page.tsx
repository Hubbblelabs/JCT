...
export async function generateStaticParams() {
  return listPublishedPageSlugs("main");
}
...
export async function generateMetadata({
  ...
  const page = await getPublishedPageBySlug({ institution: "main", slug });
...
export default async function MainDynamicPage({ params }: PageParams) {
  const { slug } = await params;
  const page = await getPublishedPageBySlug({ institution: "main", slug });
  if (!page) notFound();
  return (
