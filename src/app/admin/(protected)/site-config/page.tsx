...
function ContactForm({
  ...
  value: ContactVal;
  onChange: (v: ContactVal) => void;
}) {
  const set = (k: keyof ContactVal, v: string) =>
    onChange({ ...value, [k]: v });
  return (
    <div className="grid grid-cols-2 gap-4">
      ...
      <TextInput
        ...
        hint="Include country code, no + sign: 919361..."
      />
    </div>
  );
}
...
function StatsForm({
  ...
  value: StatsVal;
  onChange: (v: StatsVal) => void;
}) {
  const setN = (k: keyof StatsVal, v: string) =>
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <NumberInput
        ...
        value={value.yearsOfExcellence ?? 0}
        onChange={(e) => setN("yearsOfExcellence", e.target.value)}
      />
      <NumberInput
        label="Students"
        value={value.students ?? 0}
        onChange={(e) => setN("students", e.target.value)}
      />
      <NumberInput
        label="Faculty"
        value={value.faculty ?? 0}
        onChange={(e) => setN("faculty", e.target.value)}
      />
      <NumberInput
        label="Recruiters"
        value={value.recruiters ?? 0}
        onChange={(e) => setN("recruiters", e.target.value)}
      />
      <NumberInput
        label="Alumni"
        value={value.alumni ?? 0}
        onChange={(e) => setN("alumni", e.target.value)}
      />
      <NumberInput
        label="Programs"
        value={value.programs ?? 0}
        onChange={(e) => setN("programs", e.target.value)}
      />
      <NumberInput
        label="Placement Rate (%)"
        value={value.placementRate ?? 0}
        onChange={(e) => setN("placementRate", e.target.value)}
      />
      <TextInput
        label="Highest Package"
        value={value.highestPackage ?? ""}
        onChange={(e) => setS("highestPackage", e.target.value)}
        placeholder="70 LPA"
      />
      <TextInput
        label="Average Package"
        value={value.averagePackage ?? ""}
        onChange={(e) => setS("averagePackage", e.target.value)}
        placeholder="9 LPA"
      />
    </div>
  );
...
function AddressForm({
  ...
  value: AddressVal;
  onChange: (v: AddressVal) => void;
}) {
  const set = (k: keyof AddressVal, v: string) =>
    onChange({ ...value, [k]: v });
  return (
    <div className="space-y-1">
      <div className="grid grid-cols-2 gap-4">
      ...
      <TextArea
        ...
        rows={2}
        hint="The iframe src URL from Google Maps embed"
      />
    </div>
  );
}

function AccreditationsForm({
  value,
  onChange,
}: {
  value: Accreditation[];
  ...
  return (
    <div className="space-y-4">
      {safeValue.map((item, i) => (
        <div key={i} className="relative rounded-lg border border-gray-200 p-4">
          ...
          <ImageUploadInput
            label="Logo"
            value={item.logo}
            onChange={(url) => setItem(i, "logo", url)}
            uploadOnly
          />
        </div>
      ))}
      <button
        type="button"
        onClick={() =>
          onChange([...safeValue, { name: "", logo: "", description: "" }])
        }
            ...
            <div key={i} className="flex gap-2">
              ...
              <button
                ...
                onClick={() =>
                  onChange({
                    ...value,
                    titleLines: titles.filter((_, j) => j !== i),
                  })
                }
        ...
        <div className="space-y-3">
          {ctas.map((cta, i) => (
            <div key={i} className="rounded-lg border border-gray-200 p-3">
              ...
              <button
                type="button"
                onClick={() =>
                  onChange({ ...value, ctas: ctas.filter((_, j) => j !== i) })
                }
          ...
          <button
            ...
            onClick={() =>
              onChange({
                ...value,
                ctas: [...ctas, { label: "", href: "", primary: false }],
              })
            }
            className="admin-btn admin-btn-outline admin-btn-sm"
          >
        ...
        <div className="space-y-2">
            ...
            >
              ...
              <button
                ...
                onClick={() =>
                  onChange({
                    ...value,
                    trustHighlights: trust.filter((_, j) => j !== i),
                  })
                }
                className="admin-btn admin-btn-danger admin-btn-sm"
              >
          ...
          <button
            ...
            onClick={() =>
              onChange({
                ...value,
                trustHighlights: [...trust, { icon: "laurel", label: "" }],
              })
            }
            className="admin-btn admin-btn-outline admin-btn-sm"
          >
          ...
          {cards.map((card, i) => (
            <div key={i} className="rounded-lg border border-gray-200 p-3">
              ...
              <TextInput
                ...
                onChange={(e) =>
                  onChange({
                    ...value,
                    cards: cards.map((c, j) =>
                      j === i ? { ...c, highlights: e.target.value } : c,
                    ),
                  })
                }
              />
              <button
                type="button"
                onClick={() =>
                  onChange({ ...value, cards: cards.filter((_, j) => j !== i) })
                }
                className="admin-btn admin-btn-danger admin-btn-sm mt-2"
              >
                <Trash2 size={13} /> Remove
...
function HeroStatsForm({
  value,
  onChange,
}: {
  value: HeroStatVal[];
  onChange: (v: HeroStatVal[]) => void;
}) {
  const safe = Array.isArray(value) ? value : [];
...
export default function SiteConfigPage() {
  ...
  useEffect(() => {
    const cfg = configs.find(
      (c) => (c as Record<string, unknown>).config_key === selected,
    );
    if (cfg) {
  ...
  const save = async () => {
    ...
    if (r.ok) {
      ...
      if (!parsed?.details?.length)
        setMsg(parsed?.message ?? parsed?.error ?? "Error saving");
    }
    setSaving(false);
  };
  ...
  function renderForm() {
    ...
    switch (selected) {
      case "contact":
        return (
          <ContactForm
            value={currentValue as ContactVal}
      ...
      case "social":
        return (
          <SocialForm
            value={currentValue as SocialVal}
            onChange={setCurrentValue}
          />
        );
      case "stats":
        return (
          <StatsForm
            value={currentValue as StatsVal}
            onChange={setCurrentValue}
          />
        );
      case "address":
        return (
          <AddressForm
            value={currentValue as AddressVal}
            onChange={setCurrentValue}
          />
        );
      case "accreditations":
        return (
          <AccreditationsForm
            value={(currentValue as Accreditation[]) ?? []}
            onChange={setCurrentValue}
          />
        );
      case "home":
      ...
      case "engineeringMetrics":
        return (
          <MetricsForm
            value={(currentValue as MetricVal[]) ?? []}
            onChange={setCurrentValue}
          />
        );
      case "engineeringFacilities":
        return (
          <FacilitiesForm
            value={(currentValue as FacilityVal[]) ?? []}
            onChange={setCurrentValue}
          />
        );
      ...
      case "artsScienceHeroStats":
        return (
          <HeroStatsForm
            value={(currentValue as HeroStatVal[]) ?? []}
            onChange={setCurrentValue}
          />
        );
      default:
      ...
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-subtitle">
            Contact info, social links, stats, accreditations, home &
            institution content
          </p>
        </div>
            ...
            {CONFIG_KEYS.map(({ key, label }) => (
              <button
                ...
                className={`w-full rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                  selected === key
                    ? "bg-[#0a1628] font-semibold text-white"
