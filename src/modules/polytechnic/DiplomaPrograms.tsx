...
export function DiplomaPrograms() {
  ...
  return (
    <PolySection
      id="programs"
      tone="transparent"
      className="group/section border-t border-slate-100 relative overflow-hidden bg-[#f6f8f7]"
    ...
    >
      ...
      <div className="pointer-events-none absolute top-0 right-0 z-0 h-[500px] w-[500px] -translate-y-1/3 translate-x-1/3 rounded-full bg-polytechnic-accent/5 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 left-0 z-0 h-[400px] w-[400px] -translate-x-1/3 translate-y-1/3 rounded-full bg-polytechnic-dark/5 blur-3xl" />
      ...
      >
        ...
        >
          {[...diplomaPrograms, ...diplomaPrograms, ...diplomaPrograms, ...diplomaPrograms].map((prog, index) => (
            <motion.article
              key={`${prog.slug}-${index}`}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.45, delay: (index % diplomaPrograms.length) * 0.06 }}
