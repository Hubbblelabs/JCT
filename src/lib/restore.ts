...
export function isSafeStorageKey(key: unknown): key is string {
  ...
  // Traversal is a property of a path *segment*, not of the substring "..".
  // Rejecting the substring also rejected ordinary uploads whose filename
  // happens to contain it — `…at-1.43..webp` came off the WhatsApp export that
  // way — and those were dropped from every restore without a word. Checking
  // segments still catches `../`, `/..` and the empty segment in `//`.
  if (
