  ...
  return (
    <section className="section-padding bg-white overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-6 md:mb-14 gap-4 md:gap-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-11 h-11 rounded-full border border-border flex items-center justify-center text-navy hover:bg-navy hover:text-white hover:border-navy transition-all disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-navy disabled:hover:border-border"
            >
              <ChevronLeft size={18} />
            </button>
            <button
        ...
        <div
          ref={scrollRef}
          className="flex gap-5 overflow-x-auto snap-container scrollbar-hide pb-4"
        >
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="snap-item shrink-0 w-[320px] sm:w-[380px] md:w-[420px]"
            >
              <div className="h-full p-5 md:p-8 bg-surface rounded-2xl border border-border hover:border-gold/30 transition-all group">
                <Quote
                  ...
                  strokeWidth={1.5}
                />
                <p className="text-foreground/80 font-sans text-sm md:text-base leading-relaxed mb-6 min-h-[80px]">
                  &ldquo;{t.quote}&rdquo;
                </p>
                <div className="flex items-center gap-3 pt-5 border-t border-border">
                  ...
                  />
                  <div>
                    <h4 className="text-sm font-sans font-semibold text-navy">
                      {t.name}
                    </h4>
        ...
        <div className="flex justify-center gap-2 mt-6">
          {testimonials.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${i === activeIndex ? "bg-gold w-6" : "bg-border hover:bg-muted-foreground/30"}`}
            />
          ))}
        </div>
      </div>
