      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-14 md:mb-16">
          {placementStats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center p-6 bg-white rounded-2xl border border-border">
              <stat.icon size={24} className="mx-auto text-gold mb-3" strokeWidth={1.5} />
              <span className="block text-2xl md:text-3xl font-sans font-black text-navy tracking-tight mb-1">{stat.value}</span>
              <span className="text-xs font-sans font-semibold text-muted-foreground uppercase tracking-wider">{stat.label}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
