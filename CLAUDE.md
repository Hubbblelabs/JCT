...
- **The server is started with `node --require ./http-timeouts.cjs server.js`, not `node server.js`.** Node caps the time to receive an entire request body at `server.requestTimeout` — 300 s by default — and Next never assigns it, so a restore upload longer than five minutes has its socket destroyed mid-body: `Error: aborted (ECONNRESET)` in the app, `upstream prematurely closed connection while reading upstream` in nginx, "the connection dropped during upload" in the admin UI, always after roughly the same number of bytes for a given link speed. `deploy/http-timeouts.cjs` (preloaded, and copied into the runner image separately because file tracing never sees it) disables that one limit; `headersTimeout` is left alone, and nginx's per-location `client_body_timeout` remains the real bound. `pnpm start` carries the same flag. Dropping either the `COPY` or the `--require` silently reinstates the five-minute ceiling.
